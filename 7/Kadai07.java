...
public class Kadai07 extends Application {
    ...
    public void start(Stage stage) throws Exception {
		Image img = new Image( "apple_s.jpg" );		//画像ファイルロード
    	...
    	Text title0 = new Text("画像処理名:判別分析法");	//① 上部タイトルを(0,0)に配置。引数は 列,行 の順に指定。
    	grid.add(title0,0,0);
	...
	public int[][] imageProcessing(int[][] aryKido, int width, int height ) {
		...
		//クラス間分散を求める
		int kidoMax = 0;	//最大輝度値
		int kidoMin = 255;	//最小輝度値
		for(int y=0; y < height; y++ ){
			for(int x=0; x < width; x++ ){
				if( kidoMax < aryKido[x][y] ) kidoMax = aryKido[x][y];
				if( kidoMin > aryKido[x][y] ) kidoMin = aryKido[x][y];
			}
		}
		//クラス間分散を求める
		double[] classMean = new double[kidoMax - kidoMin + 1];	//クラス平均値
		double[] classCount = new double[kidoMax - kidoMin + 1];	//クラスの画素数
		for(int y=0; y < height; y++ ){
			for(int x=0; x < width; x++ ){
				int kido = aryKido[x][y];
				classMean[kido - kidoMin] += kido;	//輝度値の合計
				classCount[kido - kidoMin]++;		//画素数をカウント
			}
		}
		//クラス平均値を求める
		for(int i=0; i < classMean.length; i++ ){
			if( classCount[i] > 0 ){
				classMean[i] /= classCount[i];	//平均値を求める
			}
		}
		//クラス間分散を求める
		double classVariance = 0.0;	//クラス間分散
		for(int i=0; i < classMean.length; i++ ){
			if( classCount[i] > 0 ){
				classVariance += classCount[i] * Math.pow( classMean[i] - (kidoMax + kidoMin) / 2.0, 2 );
			}
		}
		//クラス間分散が0の場合は処理を行わない
		if( classVariance == 0.0 ){
			for(int y=0; y < height; y++ ){
				for(int x=0; x < width; x++ ){
					aryProcKido[x][y] = aryKido[x][y];	//元の輝度値をそのままコピー
				}
			}
		}
