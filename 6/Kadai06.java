...
public class Kadai06 extends Application {
    ...
    public void start(Stage stage) throws Exception {
		Image img = new Image( "boke.bmp" );		//画像ファイルロード
		...
		int aryProcKido[][] = new int[width][height];
			...
			for(int y = 1; y < height - 1; y++) {
				for(int x = 1; x < width - 1; x++) {
					aryProcKido[x][y] = (int)((double)(aryKido[x][y] - k*(aryKido[x-1][y] + aryKido[x+1][y] + aryKido[x][y-1] + aryKido[x][y+1] - 4 * aryKido[x][y])));
					aryProcKido[x][y] = Math.min(255, Math.max(0, aryProcKido[x][y]));//255以上とか0未満にならないようにする
				}
			}
			return aryProcKido;
		}
